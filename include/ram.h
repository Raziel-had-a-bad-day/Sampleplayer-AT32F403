...
void spi_message_process (void){  // send and receive list from psram
	...
	case 1:if (sound_mask.playing_sample[0]) {ram_page_read((one_play[0].pointer+psram_sample_start),download_size,1,one_play[0].buf);}
	...
	case 5 :if (sound_mask.playing_sample[5]) {ram_page_read((one_play[5].pointer+psram_sample_start),download_size,1,one_play[5].buf);}
	break;


	case 6 :if (sound_mask.playing_sample[1]) {ram_page_read((one_play[1].pointer+psram_sample_start),download_size,1,one_play[1].buf);}
	break;				////////read from psram

	case 7 :if (sound_mask.playing_sample[2]){ram_page_read((one_play[2].pointer+psram_sample_start),download_size,1,one_play[2].buf);}
	break;					////////read from psram
	case 8 :if (sound_mask.playing_sample[3]){ram_page_read((one_play[3].pointer+psram_sample_start),download_size,1,one_play[3].buf);}
	break;	// only read if enabled
	case 9 :if (sound_mask.playing_sample[4]){ram_page_read((one_play[4].pointer+psram_sample_start),download_size,1,one_play[4].buf);}
	break;	// only read if enabled
 	default:break;
